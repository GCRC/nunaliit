...
(function($) {
	...
	$.lightBox = function(settings,images_,activeImage_) {
		// Settings to configure the jQuery lightBox plugin how you like
		settings = jQuery.extend({},defaultSettings,settings);
		
		_start(activeImage_,images_);
		return;
		 ...
		 */
		function _start(selectedImage_,images_) {
			...
			for ( var i = 0; i < images_.length; i++ ) {
				var image = images_[i];
				settings.imageArray.push(new Array(image.href,image.title,-1,-1));
			}
			if( null != selectedImage_ ) {
				while ( settings.imageArray[settings.activeImage][0] != selectedImage_.href ) {
					...
					<div id="lightbox-container-image">
						...
						<div id="lightbox-loading">
							<a href="#" id="lightbox-loading-link">
								<img src="js-external/css/jquery.lightbox.resources/lightbox-ico-loading.gif">
							</a>
						</div>
					</div>
				</div>
					...
					<div id="lightbox-container-image-data">
						...
						<div id="lightbox-secNav">
							<a href="#" id="lightbox-secNav-btnClose">
								<img src="js-external/css/jquery.lightbox.resources/lightbox-btn-close.gif">
							</a>
						</div>
					</div>
				</div>
		...
		function _set_interface() {
			...
			$('body').append('<div id="jquery-overlay"></div><div id="jquery-lightbox"><div id="lightbox-container-image-box"><div id="lightbox-container-image"><img id="lightbox-image"><div style="" id="lightbox-nav"><a href="#" id="lightbox-nav-btnPrev"></a><a href="#" id="lightbox-nav-btnNext"></a></div><div id="lightbox-loading"><a href="#" id="lightbox-loading-link"><img src="' + settings.imageLocation + settings.imageLoading + '"></a></div></div></div><div id="lightbox-container-image-data-box"><div id="lightbox-container-image-data"><div id="lightbox-image-details"><span id="lightbox-image-details-caption"></span><span id="lightbox-image-details-currentNumber"></span></div><div id="lightbox-secNav"><a href="#" id="lightbox-secNav-btnClose"><img src="' + settings.imageLocation + settings.imageBtnClose + '"></a></div></div></div></div>');	
			// Get page sizes
			...
			$(window).resize(function() {
				...
				window.setTimeout(_resize_container_image_box,0);
			});
		}
		/**
		...
		function _set_image_to_view() { // show the loading
			...
			objImagePreloader.onload = function() {
				...
				settings.imageArray[settings.activeImage][2] = objImagePreloader.width;
				settings.imageArray[settings.activeImage][3] = objImagePreloader.height;
		...
		function _resize_container_image_box() {
			...
			var intImageWidth = settings.imageArray[settings.activeImage][2];
			var intImageHeight = settings.imageArray[settings.activeImage][3];
			if (settings.constrainImage) {
				var arrPageSizes = ___getPageSize();
				var ratio = 1;
				var intMaxWidth = arrPageSizes[2] - (2 * settings.containerBorderSize);
				if (intImageWidth > intMaxWidth) {
					ratio = intMaxWidth / intImageWidth;
				}
				var intMaxHeight = arrPageSizes[3] - (2 * settings.containerBorderSize) - 60 - 100;
		...
		function _start(objClicked,jQueryMatchedObj) {
			...
			if ( jQueryMatchedObj.length == 1 ) {
				var images = [{
						...
						,title:objClicked.getAttribute('title')
					}];
			} else {
				...
				for ( var i = 0; i < jQueryMatchedObj.length; i++ ) {
					images.push( {
							...
							,title: jQueryMatchedObj[i].getAttribute('title')
						});
				};
			};
			var activeImage = {
			...
			$.lightBox(settings, images, activeImage);
		};
	};
