...
//  Nunaliit
//
//  Created by JP Fiset on 12-04-11.
...
{    
    ...
    cb.delegate = self;
    cb.iniFilePath = [[NSBundle mainBundle] pathForResource:@"couchdb_app.ini" ofType:nil];
    NSAssert([cb start], @"Couchbase didn't start! Error = %@", cb.error);
    
    // Assign Couchbase to Nunaliit plugin
    [[CDVPluginNunaliit alloc] installGlobalCouchbase:cb];
...
- (BOOL) application:(UIApplication*)application handleOpenURL:(NSURL*)url 
{
    ...
    NSLog(@"Call webView with %@", jsString);
...
-(void)couchbaseMobile:(CouchbaseMobile*)couchbase didStart:(NSURL*)serverURL {
    ...
    NSString* jsString = [NSString stringWithFormat:@"window.couchLocation=\"%@\"; if(window.couchBoot)window.couchBoot();", serverURL];
    NSLog(@"Call webView with %@", jsString);
    [self.viewController.webView stringByEvaluatingJavaScriptFromString:jsString];
}
