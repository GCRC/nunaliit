...
var MobileCaptureMedia = $n2.Class({
	...
	,showSelectSource: function(){
		...
		var cameraImpl = this.getCameraImplementation();
		...
		$content.find('a').click(function(){
			...
			if( $button.hasClass('mobileMediaCaptureTakePicture') ) {
				_this._takePicture(cameraImpl.PictureSourceType.CAMERA);
				
			} else if( $button.hasClass('mobileMediaCaptureImageLibrary') ) {
				_this._takePicture(cameraImpl.PictureSourceType.PHOTOLIBRARY);
	...
	,_takePicture: function(pictureSource_){
		...
		impl.getPicture(
			...
			,{
				quality: 50
				,destinationType: impl.DestinationType.FILE_URI
			...
			if( mediaFiles ) {
				if( mediaFiles.length ) {
					...
					var fileData = {
						...
						,mimeType: mimeType
						,mediaFile: mediaFile
						,fullPath: 'file://' + mediaFile.fullPath
						,isTemporary: (1 == sourceType_)
					};
