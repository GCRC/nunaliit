...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
// There are two global definitions that dictates the start of
// the mobile application:
//
// - window.couchLocation : String  URL of the CouchDB server
//                          This string should be set by the controlling
//                          process. The mobile application waits for
//                          this string to be set before it starts.
//	
// - window.couchBoot     : This is a function declared by the mobile
//                          framework. If the controlling process detects
//                          that a change in the CouchDB server, it can
//                          inform the framework of the change by first
//                          setting the window.couchLocation variable
//                          and then calling window.couchBoot()
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
var BootStrapper = $n2.Class({
	...
	,initialize: function(opts_){
		...
		// Install call back
		window.couchBoot = function(){
			$n2.log('BootStrap recalled on location change'); 
			_this.senseCouchLocationChange();
		};
		
		this.waitForCouchLocation();
	}
	...
	,getCouchLocation: function(){
		var loc = this.options.getCouchLocationFn();
	...
	,waitForCouchLocation: function(skipWaitScreen){
	
		if( !skipWaitScreen ) {
			$n2.mobile.ShowWaitScreen('Waiting for database startup...');
		};
		...
		if( couchLocation ) {
			...
			window.setTimeout(function(){
					_this.waitForCouchLocation(true);
				}, 200);
		};
	}
	...
	,reachCouchServer: function(){
		...
		$n2.mobile.SetWaitMessage('Initializing CouchDb library');
	...
	,senseCouchLocationChange: function(){
		...
		if( lastCouchLocation !== couchLocation ) {
			...
			$n2.mobile.ShowWaitScreen('Database restart detected...');
			this.reachCouchServer();
		};
	}
	...
	,authenticateToLocalServer: function() {
		...
		var name = this.options.adminUser;
		var pw = this.options.adminPassword;
		
		this.session.login({
			name: name
			,password: pw
			,onSuccess: function(){
				_this._maintainAuthentication(name,pw);
				_this.setUpConfigDb();
			}
			,onError: function(){
	...
	,_maintainAuthentication: function(name,pw){
		var _this = this;
		window.setTimeout(
			function(){
				_this.session.login({
					...
					,password: pw
					,onSuccess: function(){
						$n2.log('Maintaining authentication');
						_this._maintainAuthentication(name,pw);
					}
					,onError: function(){
						$n2.log('Lost authentication to local database server');
						alert('Lost authentication to local database server');
						if( window
						 && window.plugins
						 ...
						 && window.plugins.CDVPLuginNunaliit.restartCouchDb ){
							window.plugins.CDVPLuginNunaliit.restartCouchDb({
								onSuccess: function(){
									$n2.log('Requested to restart database');
									_this._recoverAuthentication(name,pw);
								}
								,onError: function(err){
									alert('Failed on restarting database: '+err);
									$n2.log('Failed on restarting database: '+err);
									_this._recoverAuthentication(name,pw);
								}
							});
						} else {
							$n2.log('Unable to request database restart');
							_this._recoverAuthentication(name,pw);
						};
					}
				});
	...
	,_recoverAuthentication: function(name,pw){
		var _this = this;
		window.setTimeout(
			function(){
				_this.session.login({
					...
					,password: pw
					,onSuccess: function(){
						$n2.log('Recovered authentication');
						alert('Recovered authentication to local database server');
						_this._maintainAuthentication(name,pw);
					}
					,onError: function(){
						$n2.log('Authentication to local database server is not available');
						_this._recoverAuthentication(name,pw);
					}
				});
			}
