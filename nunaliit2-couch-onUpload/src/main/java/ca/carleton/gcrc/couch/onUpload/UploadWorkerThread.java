...
public class UploadWorkerThread extends Thread implements CouchDbChangeListener {
	...
	protected UploadWorkerThread(
			UploadWorkerSettings settings, CouchDesignDocument documentDbDesign, CouchDesignDocument submissionDbDesign,
			File mediaDir, MailNotification mailNotification, Properties uploadProperties) throws Exception {
	 ...
	 * 
	 * "submitted"
	 * Assigns value to "original" structure: size, mime type, encoding type. Set
	 * file
	 * class on attachment.
	 * -> "analyzed"
	 * 
	 * "submitted_inline"
	 * The state "submitted_inline" was implemented for the iPad application, where
	 * the
	 * file was attached to the document, but no action from the robot has been
	 * taken.
	 * In that case, download file to disk and move to regular queue (submitted)
	 * -> "submitted"
	 * 
	 * "analyzed"
	 * In this phase, the plug-ins are allowed to run. In the case of the
	 * multi-media
	...
	private void performWork(Work work) throws Exception {
		...
		FileConversionContext conversionContext = new FileConversionContextImpl(work, documentDbDesign, mediaDir);
		JSONObject doc = conversionContext.getDoc();
		if (doc == null) {
			throw new Exception("Unable to retrieve document while performing work");
		}
		String nunaliitSchema = doc.optString("nunaliit_schema");
		CouchDb couchDb = documentDbDesign.getDatabase();
	...
	private void performWaitingForUploadWork(Work work) throws Exception {
		...
		for (int i = 0, e = files.length(); i < e; ++i) {
			...
			AttachmentDescriptor requestAttachment = docDescriptor.getAttachmentDescription(effectiveAttachmentName);
		...
		if (null == attDescription) {
			...
			if (false == file.exists() || false == file.isFile()) {
				...
				if (null == uploadedAttachmentName) {
					...
					if (null != originalAttachmentName) {
						if (docDescriptor.isAttachmentDescriptionAvailable(originalAttachmentName)) {
							AttachmentDescriptor originalAttachmentDescription = docDescriptor
									.getAttachmentDescription(originalAttachmentName);
							if (originalAttachmentDescription.isFilePresent()) {
								...
								logger.info("Found original attachment for missing uploaded media: "
				...
				if (null != uploadedAttachmentName) {
					...
					logger.info("Recovered original file from database: " + uploadedAttachmentName + " to "
							+ file.getName());
				}
		...
		if (null == attDescription) {
			...
			if (false == pluginFound) {
				...
				conversionContext.uploadFile(
						attDescription.getAttachmentName(), attDescription.getMediaFile(), mimeType);
			}
	...
	private void sendVettingNotification(String docId, JSONObject doc, String attachmentName) {
		...
		try {
			...
			// String description = "A file has been submitted for approval and requires
			// your vetting action.";
	...
	private void sendDocumentCreatedEmail(JSONObject doc) {
		String docId = doc.optString("_id");
		JSONObject nunaliit_created = doc.optJSONObject("nunaliit_created", null);
		if (null == nunaliit_created) {
			logger.error("nunaliit_created missing in document " + docId);
		}
		String userId = nunaliit_created.optString("name", null);
		...
		try {
			mailNotification.sendDocumentCreatedNotification(doc, currentUser);
