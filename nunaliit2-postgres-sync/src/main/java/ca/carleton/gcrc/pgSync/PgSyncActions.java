	...
	public void recreateBaseN2Tables() {
		...
		jdbi.useHandle(handle -> {
			...
			handle.execute("CREATE TABLE n2.docs_layers (" +
					"doc_id INT NOT NULL REFERENCES n2.docs(id) ON UPDATE CASCADE ON DELETE CASCADE,\n" +
			...
			if (null != docId && !docId.startsWith("org.nunaliit")) {
				...
				if (def != null) {
					...
					jdbi.useHandle(handle -> {
						...
						fields.add(
								"doc_id INT PRIMARY KEY REFERENCES n2.docs(id) ON UPDATE CASCADE ON DELETE CASCADE");
		...
		jdbi.useHandle(h -> {
			for (JSONObject row : schemaDocs.getRows()) {
				...
				if (doc != null && doc.getJSONObject() != null) {
					...
					try {
						Integer pgId = h.select("INSERT INTO n2.docs " +
						...
						for (String l : doc.getLayers()) {
							...
							h.execute("INSERT INTO n2.docs_layers (doc_id, layer_id) VALUES (?, ?)", pgId,
									layerId);
						}
