...
public class PgSyncActions {
	...
	public void recreateBaseN2Tables() {
		jdbi.useHandle(handle -> {
			...
			handle.execute("DROP TABLE IF EXISTS n2docslayers;");
			handle.execute("DROP TABLE IF EXISTS n2layers;");
			handle.execute("DROP TABLE IF EXISTS n2docs CASCADE;");
			handle.execute("CREATE TABLE n2docs (" +
					...
					");");
			handle.execute("CREATE TABLE n2layers (" +
					"    id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,\n" +
					"    name text NOT NULL\n" +
					");");
			handle.execute("CREATE TABLE n2docslayers (" +
					"doc_id INT NOT NULL REFERENCES n2docs(id) ON UPDATE CASCADE ON DELETE CASCADE,\n" +
					"layer_id INT NOT NULL REFERENCES n2layers(id) ON UPDATE CASCADE ON DELETE CASCADE,\n" +
